...
export default function App() {
  ...
  const [warningMsg, setWarningMsg] = useState<string | null>(null); // Avertissement inactivité
  ...
  const inactivityRef  = useRef<number | null>(null);   // Timer inactivité
  const warningRef     = useRef<number | null>(null);   // Timer avertissement
  ...
  // ────────────────────────────────────────────
  // Inactivité : déconnexion après 60s sans interaction
  // ────────────────────────────────────────────
  const INACTIVITY_LIMIT = 90_000; // 90 secondes
  const WARNING_BEFORE   = 15_000; // avertissement 15s avant
  ...
  // Bip d'avertissement via Web Audio API
  const playWarningBeep = useCallback(() => {
    try {
      ...
      for (let i = 0; i < beepCount; i++) {
        ...
        osc.type      = "sine";
        osc.frequency.value = 880; // La5 — ton d'alerte
  ...
  const resetInactivity = useCallback(() => {
    ...
    // Avertissement 15s avant la fin
    warningRef.current = window.setTimeout(() => {
      setWarningMsg("⚠️ Inactivité détectée — déconnexion dans 15 secondes...");
      playWarningBeep();

      // Message visible dans le transcript
      const warningId = `inactivity-${Date.now()}`;
      setMessages((prev) => [
        ...prev,
        {
          id: warningId,
          role: "assistant",
          text: "⚠️ Aucune interaction détectée depuis 45 secondes. La session se fermera dans 15 secondes si vous ne répondez pas.",
        },
      ]);
    }, INACTIVITY_LIMIT - WARNING_BEFORE);

    // Déconnexion après 60s
    inactivityRef.current = window.setTimeout(() => {
      setWarningMsg(null);
      stopInterview();
    }, INACTIVITY_LIMIT);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [clearInactivityTimers, playWarningBeep]);
  ...
  const handleServerEvent = useCallback(
    (raw: string) => {
      ...
      switch (event.type) {
        ...
        case "input_audio_buffer.speech_started":
          ...
          isPlayingRef.current  = false;
          setIsSpeaking(false);
          resetInactivity(); // ← activité détectée
          break;
        ...
        case "response.done":
          setIsSpeaking(false);
          resetInactivity(); // ← fin de réponse = activité
          break;
    ...
    [sendEvent, enqueueAudio, upsertMessage, resetInactivity]
  );
  ...
  const startInterview = async () => {
    ...
    try {
      ...
      dc.onopen = () => {
        ...
        setStatus("connected");
        resetInactivity(); // ← démarrer le timer dès la connexion
      };
  ...
  return (
    <div className="min-h-screen bg-[#f7f5f1] text-[#2b2e27] flex flex-col">
      ...
      {/* ── Bandeau inactivité — fixe en bas ── */}
      {warningMsg && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-amber-400 border-t-2 border-amber-500 px-6 py-3 flex items-center justify-center gap-3 animate-pulse shadow-lg">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-amber-900 flex-shrink-0">
            <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/>
            <line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>
          </svg>
          <span className="text-[14px] font-semibold text-amber-900">{warningMsg}</span>
        </div>
      )}
